...
let addsym, symstr, symitr =
  ...
  let rec find s n =
    if n >= !syms then (incr syms; n) else
    if symtab.(n) = s then n else
    find s (n+1) in
...
let next =
  ...
  let rec op ch = function
    | lop :: l ->
      if lop.[0] = ch && lop.[1] = peekch ()
      then (ignore (getch ()); Op lop)
      else op ch l
  ...
  let cconst () =
    let ch = getq () in
    let qt = getch () in
    if qt <> '\'' then failwith "syntax error" else
    ILit (Char.code ch) in
...
let inss = [
  ...
  ("+", Bin [0x4801c8]); ("-", Bin [0x4891; 0x4829c8]);
  ("<<", Bin [0x4891; 0x48d3e0]); (">>", Bin [0x4891; 0x48d3f8]);
  ("<", Cmp 10); ("<=", Cmp 12); (">", Cmp 13); (">=", Cmp 11);
  ("==", Cmp 2); ("!=", Cmp 3);
  ("&", Bin [0x4821c8]);
  ("^", Bin [0x4831c8]);
  ("|", Bin [0x4809c8])
...
and unary stk =
  ...
  | Op "(" ->
    expr stk;
    ignore (next ());           (* XXX use expect here  *)
    postfix stk
  | Op "*" ->
    let (ty, i) =
      ignore (next ());
...
and postfix stk =
  ...
  | Op ("++" | "--" as op) ->
    let ol = [
      (("++", Int), 0x48ff01);  (* incq (%rcx)          *)
      (("--", Int), 0x48ff09);  (* decq (%rcx)          *)
      (("++", Chr), 0xfe01);    (* incb (%rcx)          *)
      (("--", Chr), 0xfe09);    (* decb (%rcx)          *)
    ] in
    patchlval ();
    out 0x4889c1;               (* mov %rax, %rcx       *)
    read (snd !lval);
  ...
  | Op "(" ->
    ...
    emitargs [] regs;
    pop 0;                      (* pop %rax             *)
    if !align mod 2 <> 0 then
      out 0x4883ec08;           (* sub 8, %rsp          *)
    out 0xffd0;                 (* call *%rax           *)
    if !align mod 2 <> 0 then
      out 0x4883c408            (* add 8, %rsp          *)
  | t -> unnext t
...
and expr stk =
  let rec eqexpr () =
    ...
    | Op "=" ->
      ...
      pop 1;                    (* pop %rcx             *)
      if ty = Int
      then out 0x488901         (* mov %rax, (%rcx)     *)
      else out 0x8801;          (* mov %al, (%rcx)      *)
      eqexpr ()
    | t -> unnext t in
  binary stk 10;
  eqexpr ()
...
let rec decl g n stk =
  ...
  | t when t = tokint ->
    ...
    ignore (next ());
    if !dbg then
      Printf.eprintf "end of decl (%d vars)\n" n;
    decl g (n+m) stk'
  | t ->
    unnext t;
    if not g && n <> 0 then begin
      assert (n * 8 < 256);
      out 0x4883ec;
      out (n * 8);              (* sub $n*8, %rsp       *)
      align := !align+n
    ...
    if !dbg && not g then
      prerr_endline "end of blk decls";
    (n, stk)
...
let rec stmt brk stk =
  let pexpr stk =
    ignore (next ());           (* XXX expect (         *)
    expr stk;
    ignore (next ()) in         (* XXX expect )         *)
  match next () with
  | t when t = tokif ->
    pexpr stk;
    let loc = test 0 0 in
...
let rec top () =
  ...
  | Sym f ->
    ...
    ignore (next ());           (* expect here XXX      *)
    align := 0;
    out 0x55;                   (* push %rbp  NO push!  *)
    out 0x4889e5;               (* mov %rsp, %rbp       *)
    ...
    out 0xc9c3;                 (* leave; ret           *)
    if !dbg then
      Printf.eprintf "done with function %s\n" (symstr f);
    top ()
...
let elfhdr =
  ...
  [ "\x7fELF\x02\x01\x01\x00";  (* e_ident, 64bits, little endian *)
    ...
    "\x40\x00\x00\x00\x00\x00\x00\x00"; (* e_phoff      *)
...
let elfphdr ty off sz align =
  ...
  le 32 7;                      (* p_flags, RWX         *)
  le 64 off;                    (* p_offset             *)
  le 64 (off + base);           (* p_vaddr              *)
  le 64 (off + base);           (* p_paddr              *)
  le 64 sz;                     (* p_filesz             *)
  le 64 sz;                     (* p_memsz              *)
  le 64 align                   (* p_align              *)
...
let elfgen outf =
  ...
  let itr f =
    symitr (fun i s ->
      let g = globs.(i) in
      if g.va < 0 && g.loc <> 0 then
        f s (String.length s) g.loc) in
  let va x = x+off + base in
  let patchloc i _ =
    let g = globs.(i) in
    if g.va >= 0 && g.va < base then
      patch false g.loc (va g.va)
    else if g.va >= 0 then
